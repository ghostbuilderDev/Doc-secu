...
(() => {
  ...
  const ISF_PROTECTIONS = [
    ["consignation", "Consignation caténaire", "Consignation"],
    ["interception", "Interception des circulations", "Interception"],
    ["annonce", "Annonce des circulations", "Annonce"],
    ["cloture", "Clôture limitative", "Clôture"],
    ["barriere", "Barrière défensive", "Barrière"],
  ];

  const ISF_CONDITIONS = [
    ["dfv", "Travaux sous DFV"],
    ["dfv-catenary", "Travaux sous DFV + consignation caténaire"],
    ["annonce", "Travaux sous annonce"],
    ["hors-voie", "Travaux à proximité des voies"],
    ["custom", "Autre condition à préciser"],
  ];
  ...
  function bindEvents() {
    ...
    els.isfBuilder.addEventListener("input", handleIsfBuilderInput);
    els.isfBuilder.addEventListener("change", handleIsfBuilderChange);
    els.isfBuilder.addEventListener("click", handleIsfBuilderClick);
  ...
  async function restoreLocalState() {
    ...
    state.isfPlan = normalizeIsfPlan(saved.isfPlan);
  ...
  function normalizeIsfPlan(value) {
    const phases = Array.isArray(value?.phases) ? value.phases : [];
    return { phases: phases.map((phase, index) => normalizeIsfPhase(phase, index)) };
  }

  function normalizeIsfPhase(phase = {}, index = 0) {
    const sourceTracks = Array.isArray(phase.tracks) ? phase.tracks : [];
    return {
      id: phase.id || `isf-phase-${createId()}`,
      code: normalizeText(phase.code) || `A${index + 1}`,
      title: String(phase.title || ""),
      condition: ISF_CONDITIONS.some(([value]) => value === phase.condition) ? phase.condition : "dfv",
      customCondition: String(phase.customCondition || ""),
      day: phase.day !== false,
      night: Boolean(phase.night),
      zone: String(phase.zone || ""),
      operations: String(phase.operations || ""),
      company: String(phase.company || ""),
      observations: String(phase.observations || ""),
      tracks: sourceTracks.map((track, trackIndex) => ({
        id: track.id || `isf-track-${createId()}`,
        name: normalizeText(track.name) || `Voie ${trackIndex + 1}`,
        protections: ISF_PROTECTIONS.reduce((result, [key]) => ({ ...result, [key]: Boolean(track.protections?.[key]) }), {}),
      })),
    };
  }

  function createIsfPhase(index = state.isfPlan.phases.length) {
    const defaultZone = state.dossier.lieu || "Délimité lors du briefing dans cette enveloppe";
    const defaultCompany = state.dossier.entreprise || "";
    return normalizeIsfPhase({
      code: `A${index + 1}`,
      title: state.dossier.natureTravaux || "Nouvelle phase d’activité",
      zone: defaultZone,
  ...
  function getIsfPhase(phaseId) {
    return state.isfPlan.phases.find((phase) => phase.id === phaseId);
  }

  function getIsfConditionLabel(phase) {
    if (phase.condition === "custom") return normalizeText(phase.customCondition) || "Condition à préciser";
    return ISF_CONDITIONS.find(([key]) => key === phase.condition)?.[1] || "Travaux sous DFV";
  }

  function getIsfPeriodLabel(phase) {
    if (phase.day && phase.night) return "Jour et nuit";
    if (phase.night) return "Nuit";
    return "Jour";
  }

  function countIsfOperations(phase) {
    return String(phase.operations || "").split(/\r?\n/).map(normalizeText).filter(Boolean).length;
  }

  function renderIsfBuilder() {
    const phases = state.isfPlan.phases;
    const trackCount = phases.reduce((total, phase) => total + phase.tracks.length, 0);
    const operationCount = phases.reduce((total, phase) => total + countIsfOperations(phase), 0);
    els.isfBuilder.innerHTML = `
      <section class="isf-builder-head">
        <div>
          <h2>Configurateur de phases ISF</h2>
          <p>Une phase correspond à un dispositif réel de chantier. Ajoute les voies concernées, les protections à appliquer et les opérations à autoriser : l’application prépare ensuite un tableau clair, au lieu d’une liste de zones jaunes.</p>
          <div class="isf-builder-summary">
            <span>${phases.length} phase${phases.length > 1 ? "s" : ""}</span>
            <span>${trackCount} voie${trackCount > 1 ? "s" : ""} configurée${trackCount > 1 ? "s" : ""}</span>
            <span>${operationCount} opération${operationCount > 1 ? "s" : ""}</span>
          </div>
        </div>
        <button type="button" class="button primary" data-isf-action="add-phase">＋ Ajouter une phase</button>
      </section>
      <div class="phase-list">
        ${phases.length ? phases.map((phase, index) => renderIsfPhaseCard(phase, index)).join("") : renderEmptyIsfPlan()}
      </div>`;
  }

  function renderEmptyIsfPlan() {
    return `<div class="isf-empty"><h3>Commence par ta première phase</h3><p>Exemple : A1 — travaux sous DFV, Voies 1 et 2, approvisionnement et préparation. Tu pourras ensuite dupliquer ou ajouter les phases suivantes.</p><button type="button" class="button primary" data-isf-action="add-phase">Créer la phase A1</button></div>`;
  }

  function renderIsfPhaseCard(phase, index) {
    const conditionOptions = ISF_CONDITIONS.map(([value, label]) => `<option value="${value}" ${phase.condition === value ? "selected" : ""}>${escapeHtml(label)}</option>`).join("");
    const trackRows = phase.tracks.length
      ? phase.tracks.map((track) => renderIsfTrackRow(phase, track)).join("")
      : `<div class="isf-empty"><h3>Aucune voie ajoutée</h3><p>Ajoute les voies concernées par cette phase pour sélectionner les protections correspondantes.</p></div>`;
    return `
      <article class="isf-phase-card" data-isf-phase-card="${escapeHtml(phase.id)}">
        <header class="isf-phase-titlebar">
          <div><strong>${escapeHtml(phase.code || `Phase ${index + 1}`)} — ${escapeHtml(phase.title || "Phase à nommer")}</strong><small>${escapeHtml(getIsfConditionLabel(phase))} · ${escapeHtml(getIsfPeriodLabel(phase))}</small></div>
          <div class="isf-phase-actions">
            <button type="button" class="isf-small-button" data-isf-action="duplicate-phase" data-phase-id="${escapeHtml(phase.id)}">Dupliquer</button>
            <button type="button" class="isf-small-button danger" data-isf-action="remove-phase" data-phase-id="${escapeHtml(phase.id)}">Supprimer</button>
          </div>
        </header>
        <div class="isf-phase-body">
          <div class="isf-grid">
            <label>Code de phase<input data-isf-field="code" data-phase-id="${escapeHtml(phase.id)}" maxlength="16" value="${escapeHtml(phase.code)}" placeholder="Ex. A1" /></label>
            <label>Intitulé de phase<input data-isf-field="title" data-phase-id="${escapeHtml(phase.id)}" value="${escapeHtml(phase.title)}" placeholder="Ex. Préparation et approvisionnement" /></label>
            <label>Condition<select data-isf-field="condition" data-phase-id="${escapeHtml(phase.id)}">${conditionOptions}</select></label>
          </div>
          ${phase.condition === "custom" ? `<label class="isf-section">Condition à afficher dans le tableau<input data-isf-field="customCondition" data-phase-id="${escapeHtml(phase.id)}" value="${escapeHtml(phase.customCondition)}" placeholder="Ex. Travaux sous interruption programmée" /></label>` : ""}
          <section class="isf-section">
            <h3>Période</h3>
            <div class="isf-periods">
              <label class="isf-choice"><input type="checkbox" data-isf-period="day" data-phase-id="${escapeHtml(phase.id)}" ${phase.day ? "checked" : ""} />Travaux de jour</label>
              <label class="isf-choice"><input type="checkbox" data-isf-period="night" data-phase-id="${escapeHtml(phase.id)}" ${phase.night ? "checked" : ""} />Travaux de nuit</label>
            </div>
          </section>
          <section class="isf-section">
            <h3>Zone et opérations</h3>
            <label>Zone de travail<textarea data-isf-field="zone" data-phase-id="${escapeHtml(phase.id)}" rows="2" placeholder="PK, gare, faisceau, enveloppe de briefing…">${escapeHtml(phase.zone)}</textarea></label>
            <label style="margin-top:12px">Détail des opérations <textarea data-isf-field="operations" data-phase-id="${escapeHtml(phase.id)}" rows="5" placeholder="Une opération par ligne&#10;Ex. Acheminement et chargement du matériel&#10;Ex. Pose / dépose des installations SE">${escapeHtml(phase.operations)}</textarea></label>
          </section>
          <section class="isf-section">
            <h3>Voies et protections</h3>
            <p>Ajoute les voies librement. Chaque case correspond à une protection effective sur la voie concernée.</p>
            <div class="isf-track-add"><label>Ajouter une voie<input data-isf-new-track="${escapeHtml(phase.id)}" placeholder="Ex. V1C, Voie 18, Faisceau pair" /></label><button type="button" class="button secondary" data-isf-action="add-track" data-phase-id="${escapeHtml(phase.id)}">Ajouter</button></div>
            <div class="isf-track-list">${trackRows}</div>
          </section>
          <section class="isf-section">
            <h3>Entreprise et consignes</h3>
            <label>Entreprise concernée<input data-isf-field="company" data-phase-id="${escapeHtml(phase.id)}" value="${escapeHtml(phase.company)}" placeholder="Ex. EIFFAGE Rail" /></label>
            <label style="margin-top:12px">Observations et restrictions<textarea data-isf-field="observations" data-phase-id="${escapeHtml(phase.id)}" rows="5" placeholder="Présence ASP, remise de l’ARF, restrictions caténaire, accès, PK, stockage…">${escapeHtml(phase.observations)}</textarea></label>
          </section>
          ${renderIsfPhasePreview(phase)}
        </div>
      </article>`;
  }

  function renderIsfTrackRow(phase, track) {
    const protections = ISF_PROTECTIONS.map(([key, label, shortLabel]) => `
      <label class="protection-toggle" data-protection="${key}" title="${escapeHtml(label)}"><input type="checkbox" data-isf-protection="${key}" data-phase-id="${escapeHtml(phase.id)}" data-track-id="${escapeHtml(track.id)}" ${track.protections[key] ? "checked" : ""} />${escapeHtml(shortLabel)}</label>`).join("");
    return `<div class="isf-track-row"><input class="isf-track-name" data-isf-track-name="true" data-phase-id="${escapeHtml(phase.id)}" data-track-id="${escapeHtml(track.id)}" value="${escapeHtml(track.name)}" aria-label="Nom de voie" />${protections}<button type="button" class="isf-small-button" data-isf-action="remove-track" data-phase-id="${escapeHtml(phase.id)}" data-track-id="${escapeHtml(track.id)}">Retirer</button></div>`;
  }

  function renderIsfPhasePreview(phase) {
    const headings = ISF_PROTECTIONS.map(([, , shortLabel]) => `<th>${escapeHtml(shortLabel)}</th>`).join("");
    const rows = phase.tracks.length ? phase.tracks.map((track) => `<tr><td>${escapeHtml(track.name)}</td>${ISF_PROTECTIONS.map(([key]) => `<td class="${track.protections[key] ? "yes" : "no"}">${track.protections[key] ? "✓" : "—"}</td>`).join("")}</tr>`).join("") : `<tr><td colspan="6">Ajoute au moins une voie pour afficher la matrice de protection.</td></tr>`;
    return `<section class="isf-preview"><div class="isf-preview-head">Aperçu du tableau généré · ${escapeHtml(phase.code || "Phase")} · ${escapeHtml(getIsfPeriodLabel(phase))}</div><table><thead><tr><th>Voie</th>${headings}</tr></thead><tbody>${rows}</tbody></table></section>`;
  }

  function handleIsfBuilderInput(event) {
    const target = event.target;
    const phase = getIsfPhase(target.dataset.phaseId);
    if (!phase) return;
    if (target.dataset.isfField) phase[target.dataset.isfField] = target.value;
    if (target.dataset.isfTrackName) {
      const track = phase.tracks.find((item) => item.id === target.dataset.trackId);
      if (track) track.name = target.value;
    }
    persistState();
  }

  function handleIsfBuilderChange(event) {
    const target = event.target;
    const phase = getIsfPhase(target.dataset.phaseId);
    if (!phase) return;
    if (target.dataset.isfField) phase[target.dataset.isfField] = target.value;
    if (target.dataset.isfPeriod) phase[target.dataset.isfPeriod] = target.checked;
    if (target.dataset.isfProtection) {
      const track = phase.tracks.find((item) => item.id === target.dataset.trackId);
      if (track) track.protections[target.dataset.isfProtection] = target.checked;
    }
    if (target.dataset.isfTrackName) {
      const track = phase.tracks.find((item) => item.id === target.dataset.trackId);
      if (track) track.name = target.value;
  ...
  function handleIsfBuilderClick(event) {
    const button = event.target.closest("[data-isf-action]");
    if (!button) return;
    const action = button.dataset.isfAction;
    const phase = getIsfPhase(button.dataset.phaseId);
    if (action === "add-phase") {
      state.isfPlan.phases.push(createIsfPhase());
    }
    if (action === "duplicate-phase" && phase) {
      const copy = normalizeIsfPhase({ ...phase, id: `isf-phase-${createId()}`, code: `${phase.code || "A"} bis`, tracks: phase.tracks.map((track) => ({ ...track, id: `isf-track-${createId()}`, protections: { ...track.protections } })) }, state.isfPlan.phases.length);
      state.isfPlan.phases.splice(state.isfPlan.phases.indexOf(phase) + 1, 0, copy);
    }
    if (action === "remove-phase" && phase) {
      state.isfPlan.phases = state.isfPlan.phases.filter((item) => item.id !== phase.id);
    }
    if (action === "add-track" && phase) {
      const input = Array.from(els.isfBuilder.querySelectorAll("[data-isf-new-track]")).find((item) => item.dataset.isfNewTrack === phase.id);
      const name = normalizeText(input?.value);
      if (!name) {
        if (input) input.focus();
        return;
      }
      phase.tracks.push({ id: `isf-track-${createId()}`, name, protections: ISF_PROTECTIONS.reduce((result, [key]) => ({ ...result, [key]: false }), {}) });
    }
    if (action === "remove-track" && phase) {
      phase.tracks = phase.tracks.filter((track) => track.id !== button.dataset.trackId);
    }
    persistState();
    renderIsfBuilder();
    renderExport();
  ...
  function renderFields() {
    ...
    const isIsf = template?.kind === "isf";
    els.isfBuilder.hidden = !isIsf;
    els.autofillButton.hidden = isIsf;
    els.verifyVisibleButton.hidden = isIsf;
    els.autofillButton.disabled = !isReady;
    els.verifyVisibleButton.disabled = !isReady;
    els.advancedFields.hidden = !template;
    if (!isIsf) els.advancedFields.open = true;
    ...
    if (!isReady) {
      ...
      if (isIsf) els.isfBuilder.innerHTML = `<div class="isf-empty"><h3>Préparation de l’ISF</h3><p>La trame est en cours d’analyse. Le configurateur de phases sera disponible dans quelques instants.</p></div>`;
    ...
    if (isIsf) {
      els.fieldIntro.textContent = "Construis les phases d’activité comme tu les présentes en ISF : voies, protections, opérations et observations. Les zones jaunes restantes restent disponibles uniquement dans les réglages avancés.";
  ...
  function hasIsfPlanContent() {
    return state.isfPlan.phases.some((phase) => normalizeText(phase.title) || phase.tracks.length || normalizeText(phase.operations));
  }

  function getIncompleteIsfPhases() {
    return state.isfPlan.phases.filter((phase) => !normalizeText(phase.title) || !phase.tracks.length || !normalizeText(phase.operations));
  }
  ...
  function renderExport() {
    ...
    els.exportTemplateChoices.innerHTML = templates.map((template) => {
      ...
      const isfDetails = template.kind === "isf" && hasIsfPlanContent() ? ` · ${state.isfPlan.phases.length} phase${state.isfPlan.phases.length > 1 ? "s" : ""} configurée${state.isfPlan.phases.length > 1 ? "s" : ""}` : "";
      const details = fields ? `${fields.length} zones · ${changed} modification${changed !== 1 ? "s" : ""}${isfDetails}` : "Analyse au moment de la génération";
  ...
  function createExportSummary(template) {
    ...
    if (template.kind === "isf") {
      const incomplete = getIncompleteIsfPhases().length;
      return `
        <div class="summary-row"><span>ISF · phases configurées</span><strong>${state.isfPlan.phases.length}</strong></div>
        <div class="summary-row ${incomplete ? "warn" : "good"}"><span>ISF · phases à compléter</span><strong>${incomplete}</strong></div>
        <div class="summary-row good"><span>ISF · autres zones modifiées</span><strong>${changed}</strong></div>`;
    }
    return `
  ...
  async function generateDocuments() {
    ...
    const candidates = selected.filter((template) => {
      const fields = state.scans[template.id].fields;
      const changes = fields.some((field) => hasChanged(field, getDraftEntry(template.id, field))) || (template.kind === "isf" && hasIsfPlanContent());
    ...
    const pending = candidates.reduce((total, template) => {
      if (template.kind === "isf") return total + getIncompleteIsfPhases().length;
      const fields = state.scans[template.id].fields;
  ...
  async function buildDocument(template, removeHighlights) {
    ...
    for (const [part, scannedFields] of fieldsByPart.entries()) {
      ...
      scannedFields.forEach((scannedField) => {
        ...
        const isfAutoValue = template.kind === "isf" ? getIsfAutoFieldValue(scannedField, entry) : "";
        if (hasChanged(scannedField, entry) || isfAutoValue) setGroupText(field, isfAutoValue || entry.value);
        if (removeHighlights) field.runs.forEach((run) => clearRunHighlight(run));
      });
      zip.file(part, new XMLSerializer().serializeToString(doc));
    }
    if (template.kind === "isf" && hasIsfPlanContent()) {
      const documentFile = zip.file("word/document.xml");
      if (!documentFile) throw new Error("Le document principal de l’ISF est introuvable.");
      const doc = parseXml(await documentFile.async("text"), "word/document.xml");
      injectIsfPhaseTables(doc, state.isfPlan.phases);
      zip.file("word/document.xml", new XMLSerializer().serializeToString(doc));
    }
    return zip.generateAsync({ type: "blob", compression: "DEFLATE", compressionOptions: { level: 6 } });
  }

  function getIsfAutoFieldValue(field, entry) {
    if (hasChanged(field, entry)) return "";
    const source = normalizeText(field.source);
    if (!/^travaux\s*:/i.test(source)) return "";
    const label = normalizeText(state.dossier.natureTravaux || state.dossier.operation || state.isfPlan.phases.map((phase) => phase.title).filter(Boolean).join(" · "));
    return label ? `TRAVAUX : ${label}` : "";
  }

  function injectIsfPhaseTables(doc, phases) {
    const legacyTables = Array.from(doc.getElementsByTagNameNS(WORD_NS, "tbl")).filter(isIsfLegacyPhaseTable);
    if (!legacyTables.length) {
      throw new Error("Les tableaux de phases d’activité n’ont pas été trouvés dans la trame ISF.");
    }
    const anchor = legacyTables[0];
    const parent = anchor.parentNode;
    if (!parent) throw new Error("Impossible de placer les tableaux de phases dans la trame ISF.");

    phases.forEach((phase, index) => {
      if (index) parent.insertBefore(createIsfPhasePageBreak(doc), anchor);
      parent.insertBefore(createIsfPhaseTable(doc, phase), anchor);
    });
    legacyTables.forEach((table) => table.parentNode?.removeChild(table));
  }

  function isIsfLegacyPhaseTable(table) {
    const text = normalizeForSearch(Array.from(table.getElementsByTagNameNS(WORD_NS, "t")).map((node) => node.textContent || "").join(" "));
    return text.includes("mesures de protection") && /phases\s+d.{0,8}activit/.test(text);
  }

  function createIsfPhasePageBreak(doc) {
    const paragraph = wordElement(doc, "p");
    const run = wordElement(doc, "r");
    const breakNode = wordElement(doc, "br");
    setWordAttribute(breakNode, "type", "page");
    run.appendChild(breakNode);
    paragraph.appendChild(run);
    return paragraph;
  }

  function createIsfPhaseTable(doc, rawPhase) {
    const phase = normalizeIsfPhase(rawPhase);
    const widths = [600, 3500, 900, 800, 800, 800, 800, 800];
    const table = wordElement(doc, "tbl");
    const properties = wordElement(doc, "tblPr");
    properties.appendChild(wordElement(doc, "tblW", { w: sumWordWidths(widths), type: "dxa" }));
    properties.appendChild(wordElement(doc, "tblLayout", { type: "fixed" }));
    properties.appendChild(createWordTableBorders(doc));
    properties.appendChild(createWordCellMargins(doc));
    const look = wordElement(doc, "tblLook", { val: "04A0", firstRow: "1", firstColumn: "0", noHBand: "1", noVBand: "1" });
    properties.appendChild(look);
    table.appendChild(properties);

    const grid = wordElement(doc, "tblGrid");
    widths.forEach((width) => grid.appendChild(wordElement(doc, "gridCol", { w: width })));
    table.appendChild(grid);

    const phaseFill = "7D1F4E";
    const activityFill = "EEE6EC";
    const detailFill = "F8F6F8";
    const protectionFills = ["D9EAF7", "FCE4D6", "FFF2CC", "D9EAD3", "D9D2E9"];
    const protectionHeaders = ["Consign.\ncatén.", "Interc.\ncircul.", "Annonce", "Clôture\nlimit.", "Barrière\ndéfens."];
    const tracks = phase.tracks.length ? phase.tracks : [{ name: "Voie à préciser", protections: {} }];
    const operationLines = normalizeText(phase.operations) ? phase.operations : "À préciser";
    const zone = normalizeText(phase.zone) ? phase.zone : "À préciser";
    const company = normalizeText(phase.company || state.dossier.entreprise) || "À préciser";
    const observations = normalizeText(phase.observations) || "À préciser";

    table.appendChild(createWordTableRow(doc, [
      createWordTableCell(doc, "N°", { width: widths[0], fill: phaseFill, color: "FFFFFF", bold: true, align: "center", size: 16 }),
      createWordTableCell(doc, "OPÉRATION OU PHASE D’ACTIVITÉ", { width: sumWordWidths(widths, 1, 2), span: 2, fill: phaseFill, color: "FFFFFF", bold: true, align: "center", size: 16 }),
      createWordTableCell(doc, "DISPOSITIF DE SÉCURITÉ", { width: sumWordWidths(widths, 3, 5), span: 5, fill: phaseFill, color: "FFFFFF", bold: true, align: "center", size: 16 }),
    ], true));

    table.appendChild(createWordTableRow(doc, [
      createWordTableCell(doc, phase.code || "A", { width: widths[0], fill: "E8D8E1", color: phaseFill, bold: true, align: "center", size: 20 }),
      createWordTableCell(doc, `${phase.title || "Phase à préciser"}\n${getIsfConditionLabel(phase)} · ${getIsfPeriodLabel(phase)}`, { width: sumWordWidths(widths, 1, 2), span: 2, fill: activityFill, bold: true, size: 17 }),
      ...protectionHeaders.map((label, index) => createWordTableCell(doc, label, { width: widths[index + 3], fill: protectionFills[index], bold: true, align: "center", size: 12 })),
    ], true));

    tracks.forEach((track, index) => {
      const protections = track.protections || {};
      const detail = index === 0 ? `ZONE DE TRAVAIL\n${zone}\n\nDÉTAIL DES OPÉRATIONS\n${operationLines}` : "";
      table.appendChild(createWordTableRow(doc, [
        createWordTableCell(doc, "", { width: widths[0], fill: "FFFFFF" }),
        createWordTableCell(doc, detail, { width: widths[1], fill: index === 0 ? detailFill : "FFFFFF", size: 16 }),
        createWordTableCell(doc, track.name || "Voie à préciser", { width: widths[2], bold: true, align: "center", size: 16 }),
        ...ISF_PROTECTIONS.map(([key], protectionIndex) => createWordTableCell(doc, protections[key] ? "X" : "", { width: widths[protectionIndex + 3], fill: protections[key] ? protectionFills[protectionIndex] : "FFFFFF", color: "202124", bold: true, align: "center", size: 20 })),
      ]));
    });

    table.appendChild(createWordTableRow(doc, [
      createWordTableCell(doc, "", { width: widths[0] }),
      createWordTableCell(doc, `ENTREPRISE : ${company}`, { width: sumWordWidths(widths, 1, 7), span: 7, fill: "F3F0F2", bold: true, size: 16 }),
    ]));
    table.appendChild(createWordTableRow(doc, [
      createWordTableCell(doc, "", { width: widths[0] }),
      createWordTableCell(doc, `OBSERVATIONS : ${observations}`, { width: sumWordWidths(widths, 1, 7), span: 7, size: 16 }),
    ]));
    return table;
  }

  function createWordTableBorders(doc) {
    const borders = wordElement(doc, "tblBorders");
    ["top", "left", "bottom", "right", "insideH", "insideV"].forEach((edge) => {
      borders.appendChild(wordElement(doc, edge, { val: "single", sz: "6", space: "0", color: "6E6570" }));
    });
    return borders;
  }

  function createWordCellMargins(doc) {
    const margins = wordElement(doc, "tblCellMar");
    [["top", 80], ["left", 105], ["bottom", 80], ["right", 105]].forEach(([side, size]) => {
      margins.appendChild(wordElement(doc, side, { w: size, type: "dxa" }));
    });
    return margins;
  }

  function createWordTableRow(doc, cells, isHeader = false) {
    const row = wordElement(doc, "tr");
    if (isHeader) {
      const properties = wordElement(doc, "trPr");
      properties.appendChild(wordElement(doc, "tblHeader", { val: "true" }));
  ...
  function createWordTableCell(doc, text, options = {}) {
    const cell = wordElement(doc, "tc");
    const properties = wordElement(doc, "tcPr");
    properties.appendChild(wordElement(doc, "tcW", { w: options.width || 0, type: "dxa" }));
    if (options.span && options.span > 1) properties.appendChild(wordElement(doc, "gridSpan", { val: options.span }));
    if (options.fill && options.fill !== "FFFFFF") properties.appendChild(wordElement(doc, "shd", { val: "clear", color: "auto", fill: options.fill }));
    properties.appendChild(wordElement(doc, "vAlign", { val: "center" }));
    cell.appendChild(properties);
    cell.appendChild(createWordParagraph(doc, text, options));
    return cell;
  }

  function createWordParagraph(doc, text, options = {}) {
    const paragraph = wordElement(doc, "p");
    const properties = wordElement(doc, "pPr");
    properties.appendChild(wordElement(doc, "spacing", { before: "0", after: "0", line: "220", lineRule: "auto" }));
    if (options.align) properties.appendChild(wordElement(doc, "jc", { val: options.align }));
    paragraph.appendChild(properties);
    const run = wordElement(doc, "r");
    const runProperties = wordElement(doc, "rPr");
    runProperties.appendChild(wordElement(doc, "rFonts", { ascii: "Arial", hAnsi: "Arial", cs: "Arial" }));
    runProperties.appendChild(wordElement(doc, "sz", { val: options.size || 16 }));
    if (options.bold) runProperties.appendChild(wordElement(doc, "b"));
    if (options.color) runProperties.appendChild(wordElement(doc, "color", { val: options.color }));
    run.appendChild(runProperties);
    const lines = String(text ?? "").replace(/\r\n/g, "\n").split("\n");
    lines.forEach((line, index) => {
      if (index) run.appendChild(wordElement(doc, "br"));
      if (!line) return;
      const textNode = wordElement(doc, "t");
      if (/^\s|\s$/.test(line)) textNode.setAttributeNS(XML_NS, "xml:space", "preserve");
  ...
  function wordElement(doc, localName, attributes = {}) {
    const element = doc.createElementNS(WORD_NS, `w:${localName}`);
    Object.entries(attributes).forEach(([name, value]) => setWordAttribute(element, name, value));
    return element;
  }
  ...
  function sumWordWidths(widths, start = 0, count = widths.length) {
  ...
  function setConfirmDialogVisible(visible) {
    els.confirmDialog.hidden = !visible;
    els.confirmDialog.style.display = visible ? "grid" : "none";
  }
  ...
  if (window.__DOCU_TEST__) {
    Object.assign(window.__DOCU_TEST__, {
      injectIsfPhaseTables,
      normalizeIsfPlan,
    });
  }
})();
